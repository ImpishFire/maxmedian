...
names_xp = ['xp'+str(i+3)+'_' for i in range(1)]  # change both names_xp and xp_list if running less xp
xp_list = [xp3]#, xp2, xp3, xp4
algs = ['ExtremeHunter', 'MaxMedian']  # Selection of algorithms to test , 'Threshold_Ascent'
# T_list = [1000, 2500, 5000, 7500, 10000, 15000, 20000, 30000, 50000]  # Selection of times from the paper
# T_list = [1000, 2000, 3000, 4000, 5000]  # Sub-selection of times for testing
...
if __name__ == '__main__':
    for i, xp in enumerate(xp_list):
        ...
        for T in T_list:
            ...
            mc_counts.append(100*mc_count[4]/T)
            mc_counts_eh.append(100*mc_count_eh[4]/T)
            print(mc_count)
        # file = open("mc_counts_h3", "wb")
        # np.save(file, mc_counts)
        # file.close
        # t = np.linspace(10, 5000, num = 200)
        ...
        #     fig, ax = plt.subplots()
        #     ax.plot(t, smooth_mc, color='blue', label='max-median')
        #     ax.plot(t, smooth_mc_eh, color='red', label='extremehunter')
